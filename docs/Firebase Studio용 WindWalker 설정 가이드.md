...
### 현재 상황
- Firebase Studio는 **Nix 환경**을 사용
- Docker daemon이 실행되지 않음
- `docker-compose`가 설치되지 않음
...
Firebase Studio 환경 특성상 Docker보다는 **직접 실행**이 더 안정적입니다.
...
Firebase Studio에서 iframe으로 로컬호스트에 접근하려면 **포트를 올바르게 노출**해야 합니다.

#### 3.1 Firebase Studio 설정 파일 확인

```yaml
# firebase-studio.yaml 또는 dev.nix
