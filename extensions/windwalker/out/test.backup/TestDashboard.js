...
class TestDashboard {
     ...
     */
    async generateIntegratedDashboard(currentReport) {
...
<body>
     ...
     * 히스토리 대시보드 HTML 생성
     */
    async generateHistoryDashboard(data) {
...
<body>
     ...
     * 스크린샷 갤러리 HTML 생성
     */
    async generateScreenshotGallery(data) {
...
<div class="metrics-grid">
    ...
    async generateScreenshotGrid(screenshots) {
...
<div class="screenshot-grid">
    ...
    async generateModuleScreenshots(report) {
        ...
        for (const module of modules) {
            ...
            await fs.writeFile(path.join(this.outputDir, screenshotPath), dummyImageData);
        }
        return screenshots;
    }
    async generateDummyScreenshot(moduleName) {
        // SVG로 더미 스크린샷 생성
        const svg = `
...
`;
    ...
    async saveTestHistory(currentReport) {
        try {
            const historyPath = path.join(this.outputDir, 'test-history.json');
            let history = { reports: [] };
            try {
