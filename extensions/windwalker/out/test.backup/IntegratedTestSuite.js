...
class IntegratedTestSuite {
    ...
    async setupTestEnvironment() {
        ...
        try {
            ...
            await fs.writeFile(path.join(this.testWorkspace, 'package.json'), JSON.stringify(packageJson, null, 2));
            // 테스트용 소스 파일 생성
            await fs.mkdir(path.join(this.testWorkspace, 'src'), { recursive: true });
            await fs.writeFile(path.join(this.testWorkspace, 'src', 'test.ts'), '// Test file for WindWalker integration tests\nexport const testVar = "test";');
    ...
    async testServiceRegistryInitialization() {
    ...
    async testGitIntegrationManager() {
    ...
    async testConversationDatabase() {
        await this.runTest('ConversationDatabase_Full', async () => {
            const db = new ConversationDatabase_1.ConversationDatabase();
            await db.initialize();
            // 테스트 대화 생성
            const conversationId = await db.createConversation('test-user-123', 'react', { templateUsed: 'test-template' });
            // 메시지 저장 테스트
            const messageId = await db.saveMessage(conversationId, 'user', 'Create a test component', {
    ...
    async testConversationHistoryTracker() {
        await this.runTest('ConversationHistoryTracker_Full', async () => {
            ...
            // 테스트 대화 변경 추적
            const entry = await tracker.trackConversationChange('test-conversation-456', 'test-message-789', 'test-user-123', 'Create a new component', 'Component created successfully', ['src/TestComponent.tsx'], 'create');
            // 히스토리 조회 테스트
    ...
    // === Integration Tests ===
    async testEndToEndConversationFlow() {
        await this.runTest('E2E_ConversationFlow', async () => {
            ...
            // 2. Mock WebView
            const mockWebView = {
                postMessage: async (message) => ({ type: message.type, success: true })
            };
            // 3. 대화 시작
    ...
    async testGitConversationLinking() {
        await this.runTest('Git_Conversation_Linking', async () => {
            ...
            // 2. 메시지 저장
            const messageId = await db.saveMessage(conversationId, 'user', 'Add a new utility function');
            // 3. Git 커밋 생성 (실제 파일 변경이 있다면)
            try {
                const commitResult = await gitManager.createAIConversationCommit(conversationId, messageId, 'Add utility function', 'Function added successfully', ['src/utils.ts'], {
                    model: 'test-model',
                    confidence: 0.95,
                    processingTime: 200
                });
                // 4. DB에 Git 연결 저장
                await db.linkGitCommit(conversationId, messageId, {
    ...
    async testPersonalizationFlow() {
        await this.runTest('Personalization_Flow', async () => {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                await db.saveMessage(conversationId, 'user', `Test request ${i + 1}: Create component`, { messageMetadata: { testIndex: i } });
                await db.saveMessage(conversationId, 'ai', `Component created for request ${i + 1}`, {
                    aiMetadata: {
                        model: 'test-model',
     ...
     * 통합 대시보드 생성 및 링크 공유
     */
    async generateAndShareDashboard(report) {
        try {
            ...
            const [finalReportPath, historyDashboardPath, screenshotGalleryPath] = await dashboard.generateIntegratedDashboard(report);
            // 파일 URL로 변환
            ...
            const choice = await vscode.window.showInformationMessage(`🎯 WindWalker 테스트 완료! 결과: ${report.passed}/${report.totalTests} 성공`, openDashboard);
