...
class BasicSystemTest {
    ...
    async testPhase1Environment() {
        ...
        try {
            ...
            this.addResult('Phase 1: Environment Setup', true, Date.now() - startTime, `✅ Context OK, Workspace: ${hasWorkspace ? 'Yes' : 'No'}`);
        }
        catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            this.addResult('Phase 1: Environment Setup', false, Date.now() - startTime, `❌ Environment test failed: ${errorMsg}`);
        }
    }
    async testPhase2FileSystem() {
        var _a;
        const startTime = Date.now();
        try {
            // 기본 파일시스템 접근 테스트
            ...
            if (workspace) {
                ...
                this.addResult('Phase 2: File System Access', canAccess, Date.now() - startTime, `✅ Workspace access: ${workspacePath}`);
            }
            else {
                this.addResult('Phase 2: File System Access', true, Date.now() - startTime, '⚠️ No workspace - file system test skipped');
            }
        }
        catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            this.addResult('Phase 2: File System Access', false, Date.now() - startTime, `❌ File system test failed: ${errorMsg}`);
        }
    }
    async testPhase3BuildSystem() {
        var _a;
        const startTime = Date.now();
        try {
            ...
            this.addResult('Phase 3: Build System Detection', true, // 빌드 시스템이 없어도 정상
            Date.now() - startTime, `✅ Build system: ${buildSystemType}, package.json: ${hasPackageJson}`);
        }
        catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            this.addResult('Phase 3: Build System Detection', false, Date.now() - startTime, `❌ Build system test failed: ${errorMsg}`);
        }
    }
    async testPhase4MessageSystem() {
        const startTime = Date.now();
        try {
            ...
            this.addResult('Phase 4: Message System', true, Date.now() - startTime, '✅ Message structure validation passed');
        }
        catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            this.addResult('Phase 4: Message System', false, Date.now() - startTime, `❌ Message system test failed: ${errorMsg}`);
        }
    }
    async testPhase5Integration() {
        ...
        try {
            ...
            const isActive = (extension === null || extension === void 0 ? void 0 : extension.isActive) || false;
            this.addResult('Phase 5: Integration Test', true, Date.now() - startTime, `✅ VS Code API: ${apiAccessCount}/4, Extension Active: ${isActive}`);
        }
        catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            this.addResult('Phase 5: Integration Test', false, Date.now() - startTime, `❌ Integration test failed: ${errorMsg}`);
        }
    }
    addResult(testName, success, duration, message) {
        this.results.push({
            testName,
            success,
            duration,
