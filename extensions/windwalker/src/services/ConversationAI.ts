...
export interface UserPreferences {
  preferredStyle: 'modern' | 'classic' | 'creative' | 'minimal';
  colorPreference: 'bright' | 'dark' | 'neutral' | 'colorful';
...
export interface SuggestedAction {
  id: string;
  type: 'apply-template' | 'modify-style' | 'add-content' | 'change-layout';
  label: string;
  description: string;
  parameters: any;
...
export class ConversationAI implements ServiceInterface {
   ...
   * 의도 기반 AI 응답 생성
   */
  async generateResponse(intent: UserIntent, userMessage: string): Promise<AIResponse> {
      ...
      switch (entity.type) {
        case 'template-style':
          if (['modern', 'classic', 'creative', 'minimal'].includes(entity.value)) {
            updatedContext.userPreferences.preferredStyle = entity.value as any;
          }
          break;
        case 'color':
   ...
   * 사용자 선호도 기반 개인화
   */
  personalizeResponse(response: AIResponse, context: ConversationContext): AIResponse {
    ...
    if (context.previousIntents.length > 0) {
      ...
      personalizedResponse.followUpQuestions.unshift(
        this.generateContextualQuestion(recentIntent)
      );
    }
  ...
  private classifyPrimaryIntent(message: string): IntentType {
    let bestIntent: IntentType = 'help-request';
    let maxScore = 0;
  ...
  private classifySecondaryIntents(message: string): IntentType[] {
  ...
  private generateResponseMessage(intent: UserIntent, userMessage: string): string {
  ...
  private generateSuggestedActions(intent: UserIntent): SuggestedAction[] {
  ...
  private generateTemplateRecommendations(intent: UserIntent): string[] {
  ...
  private generateFollowUpQuestions(intent: UserIntent): string[] {
    ...
    switch (intent.primary) {
      case 'create-website':
        questions.push(
          '어떤 색상 테마를 선호하시나요?',
          '웹사이트에 포함하고 싶은 페이지가 있나요?'
        );
        break;

      case 'apply-template':
        questions.push(
          '템플릿의 어떤 부분을 수정하고 싶으신가요?',
          '내용을 어떻게 변경할까요?'
        );
        break;
  ...
  private updateColorPreference(preferences: UserPreferences, colorValue: string): void {
  ...
  private generateAdvancedActions(): SuggestedAction[] {
    return [
      {
  ...
  private generateContextualQuestion(recentIntent: IntentType): string {
    switch (recentIntent) {
      case 'create-website':
  ...
  private inferCategoryFromEntities(entities: ExtractedEntity[]): string {
    const websiteTypeEntity = entities.find(e => e.type === 'website-type');
    
    if (websiteTypeEntity) {
      const value = websiteTypeEntity.value.toLowerCase();
      if (value.includes('레스토랑')) return 'restaurant';
      if (value.includes('포트폴리오')) return 'portfolio';
      if (value.includes('블로그')) return 'blog';
      if (value.includes('쇼핑')) return 'ecommerce';
      if (value.includes('회사')) return 'business';
    }
