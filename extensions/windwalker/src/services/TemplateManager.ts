...
export interface TemplateStructure {
  layout: 'single-page' | 'multi-page' | 'spa';
  sections: TemplateSection[];
  navigation: 'header' | 'sidebar' | 'footer' | 'none';
  responsive: boolean;
}
...
export interface TemplateSection {
  id: string;
  name: string;
  type: 'header' | 'hero' | 'content' | 'sidebar' | 'footer' | 'gallery' | 'contact' | 'testimonials';
  required: boolean;
...
export class TemplateManager implements ServiceInterface {
   ...
   * 의도 기반 템플릿 추천
   */
  async recommendTemplates(request: RecommendationRequest): Promise<Template[]> {
    try {
   ...
   * 템플릿 적용
   */
  async applyTemplate(templateId: string, customizations?: any): Promise<ApplyResult> {
    try {
  ...
  private inferCategoryFromIntent(intent: string): TemplateCategory {
  ...
  private filterByRequirements(templates: Template[], requirements?: RecommendationRequest['requirements']): Template[] {
    if (!requirements) return templates;
  ...
  private scoreTemplatesByPreferences(
    templates: Template[], 
    preferences?: RecommendationRequest['preferences']
  ): Array<{ template: Template; score: number }> {
  ...
  private generateHTMLContent(template: Template, customizations?: any): string {
...
<body>
  ...
  private generateSectionHTML(template: Template, customizations?: any): string {
  ...
  private generateHeaderHTML(template: Template, customizations?: any): string {
    const siteName = customizations?.siteName || template.name;
    return `
    <header class="header">
        <nav class="nav">
            <div class="nav-brand">
  ...
  private generateHeroHTML(template: Template, customizations?: any): string {
    const heroTitle = customizations?.heroTitle || `Welcome to ${template.name}`;
    const heroSubtitle = customizations?.heroSubtitle || template.description;
  ...
  private generateContentHTML(template: Template, section: TemplateSection, customizations?: any): string {
    return `
  ...
  private generateGalleryHTML(template: Template, customizations?: any): string {
    return `
  ...
  private generateContactHTML(template: Template, customizations?: any): string {
    return `
  ...
  private generateFooterHTML(template: Template, customizations?: any): string {
  ...
  private generateCSSContent(template: Template, customizations?: any): string {
...
@media (max-width: 768px) {
  ...
  private generateJSContent(template: Template, customizations?: any): string {
    return `// Generated by WindWalker AI - ${template.name}
// Template: ${template.id}
