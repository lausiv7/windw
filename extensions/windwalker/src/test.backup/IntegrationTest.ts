...
import { EnhancedMessageBridge, EnhancedWindWalkerMessage } from '../core/EnhancedMessageBridge';
...
import { AIWorkflowRouter } from '../core/AIWorkflowRouter';
...
export class IntegrationTest {
  ...
  async runPhase1IntegrationTests(): Promise<TestReport> {
  ...
  private async testFeatureFlagSystem(): Promise<void> {
    ...
    try {
      const featureFlagManager = await this.serviceRegistry.getService<FeatureFlagManager>('FeatureFlagManager');
      ...
      this.addTestResult(
        ...
        true,
        Date.now() - startTime,
        `✅ Essential flags enabled, toggle test passed`
      );
  ...
  private async testEnhancedMessageBridge(): Promise<void> {
    ...
    try {
      const messageBridge = await this.serviceRegistry.getService<EnhancedMessageBridge>('EnhancedMessageBridge');

      // Mock 웹뷰
      const mockWebview = this.createMockWebview();
      
      // 테스트 메시지 생성
      const testMessage: EnhancedWindWalkerMessage = {
      ...
      this.addTestResult(
        'Enhanced Message Bridge',
        true,
        Date.now() - startTime,
        `✅ Message processing successful, response sent`
      );
      ...
      this.addTestResult(
        ...
        false,
        Date.now() - startTime,
        `❌ Message bridge test failed: ${error.message}`
      );
    }
  }

  private async testAIWorkflowRouter(): Promise<void> {
    const startTime = Date.now();

    try {
      const aiRouter = new AIWorkflowRouter(this.context);
      await aiRouter.initialize();

      // 워크플로우 시작 테스트
      const session = await aiRouter.startWorkflowSession(
      ...
      this.addTestResult(
        ...
        true,
        Date.now() - startTime,
        `✅ Workflow session created, intent analysis: ${result.data.intent}`
      );
      ...
      this.addTestResult(
        ...
        false,
        Date.now() - startTime,
        `❌ AI workflow test failed: ${error.message}`
      );
    }
  }
  ...
  private async testConversationHistoryTracker(): Promise<void> {
    ...
    try {
      const historyTracker = await this.serviceRegistry.getService<ConversationHistoryTracker>('ConversationHistoryTracker');
      ...
      const entry = await historyTracker.trackConversationChange(
        conversationId,
        messageId,
        userId,
        'Create a homepage',
        'Homepage created with modern design',
      ...
      this.addTestResult(
        ...
        true,
        Date.now() - startTime,
        `✅ Conversation tracked, ${history.length} entries in history`
      );
      ...
      this.addTestResult(
        ...
        false,
        Date.now() - startTime,
        `❌ History tracker test failed: ${error.message}`
      );
    }
  }

  private async testGitIndexedDBIntegration(): Promise<void> {
    const startTime = Date.now();
    ...
    try {
      ...
      const gitTestMessage: EnhancedWindWalkerMessage = {
        type: 'file:create',
        data: {
      ...
      // Git 관련 응답이 있는지 확인
      const gitResponses = mockWebview.sentMessages.filter(msg => 
        msg.type === 'git:committed' || msg.type === 'git:error'
      );
      ...
      this.addTestResult(
        'Git+IndexedDB Integration',
        true,
        Date.now() - startTime,
        `✅ Git integration message processed, ${gitResponses.length} Git responses`
      );
      ...
      this.addTestResult(
        ...
        false,
        Date.now() - startTime,
        `❌ Git integration test failed: ${error.message}`
      );
    }
  }

  private async testConversationRevertFeature(): Promise<void> {
    const startTime = Date.now();
    ...
    try {
      ...
      // 여러 단계의 변경사항 시뮬레이션
      await historyTracker.trackConversationChange(
        conversationId,
        'msg-001',
        'revert-test-user',
        'Create homepage',
      ...
      this.addTestResult(
        'Conversation Revert Feature',
        true,
        Date.now() - startTime,
        `✅ Revert preview generated, revert operation: ${revertResult.message}`
      );
      ...
      this.addTestResult(
        ...
        false,
        Date.now() - startTime,
        `❌ Revert feature test failed: ${error.message}`
      );
    }
  }

  private async testPersonalizationSystem(): Promise<void> {
    const startTime = Date.now();
    ...
    try {
      ...
      this.addTestResult(
        ...
        true,
        Date.now() - startTime,
        `✅ Insights generated for user, ${insights.recommendedNextActions.length} recommendations`
      );
      ...
      this.addTestResult(
        ...
        false,
        Date.now() - startTime,
        `❌ Personalization test failed: ${error.message}`
      );
    }
  }
  ...
  private addTestResult(testName: string, success: boolean, duration: number, message: string, details?: any): void {
